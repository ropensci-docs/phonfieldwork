...
The output tables from the `phonfieldwork`’s functions (e. g.
...
morpheme/observation or aother unit. In order to merge it to a more
common representation we will use `tidyverse` functions (mainly `dplyr`
and `tidyr` packages). This text will help you to achieve some results,
but it is better to spend some times learning data manipulation with
`dplyr` and `tidyr`.
...
The standard sound annotation formats consisnt of tiers with parallel
annotation:
...
As we see this table has a long format structure: each observation has
its own row. We can select the first two rows with the
[`filter()`](https://rdrr.io/r/stats/filter.html) function, remove all
unnecessary columns with the `select()` function and spread everything
in a table with the `pivot_wider()` function:
...
Imagine that we obtained the first result from
[`flextext_to_df()`](https://docs.ropensci.org/phonfieldwork/reference/flextext_to_df.md):
...
As we can see from `df` print there are three indices in the dataset:
`p_id` – paragraph id, `s_id` – sentence id and `w_id` – word id.
...
The first [`filter()`](https://rdrr.io/r/stats/filter.html) removes some
garbage rows that are present in our example flextext. The `select()`
function selects only six important columns from 15 presented in the
dataset. The `group_by()` and `summarise()` merge all text from `txt`
variable and all glosses from `gls` variable together. Pipe operater
`%>%` make it possible to pass the result from the previous funstion as
an input to the following one.

So now we can use the same code in order to merge everything into
sentences:
...
Here with the [`filter()`](https://rdrr.io/r/stats/filter.html) function
we remove all empty glosses, then we calculate and sort them according
to frequency, and in the end we take top six glosses with the `top_n()`
function. We can even visualis it with the `ggplot2` package:
